...
- [x] **Phase 1: Upload-to-Audio Spike (TTS/ROCm De-risk)** - User uploads a .txt file and gets back a real, audible audio file generated by self-hosted Qwen TTS on the actual GPU (completed 2026-07-09)
- [x] **Phase 2: LLM Cast Detection & Review Wizard** - User uploads .txt or .epub, the app auto-casts characters and segments the text, and the user reviews/edits the cast and assigns/previews voices (completed 2026-07-10)
- [x] **Phase 3: Editable Table, Full Generation Pipeline, Persistence & Deployment** - User edits the full segment table, generates/regenerates audio with caching and resumable progress, saves/reopens projects, and reaches the app privately over Tailscale (completed 2026-07-12)
...
**Wave 4** *(blocked on Wave 3 completion)*
  ...
  - [x] 02-05-PLAN.md — Frontend cast-review wizard: upload UI, SSE analyzing, single-page cast cards + instant preview, read-only segment preview (ING-02/WIZ-01..05)
...
**Wave 1**
  ...
  - [x] 03-01-PLAN.md — Editable segment table + per-row generate with content-hash cache; front-loads all Phase 3 schema fields (TBL-01/02/04, GEN-02/03)

**Wave 2** *(blocked on Wave 1)*

  - [x] 03-02-PLAN.md — Bulk row select + validated bulk-reassign toolbar (TBL-03)

**Wave 3** *(blocked on Wave 1; sequential with Wave 2 via shared main.py/client.ts)*

  - [x] 03-03-PLAN.md — Resumable batch generation, live SSE progress, right-side config panel + join (GEN-05, CFG-01/02/03)

**Wave 4** *(blocked on Waves 1+3)*

  - [x] 03-04-PLAN.md — Project list + reopen persistence, landing screen (PERS-01/02)
...
*Wave 1 (parallel — no file overlap):*
  ...
  - [x] 03-06-PLAN.md — Backend persistent data volume + restart-resilient Quadlet units, VM-verified (DEPL-02/PERS-01/02) [blocker: test 1]
  - [x] 03-07-PLAN.md — Recover from stale/stuck analysis stream (permanent-404 vs transient drop) (PERS-01/02) [blocker: test 3]
  - [x] 03-08-PLAN.md — Backend generation lifecycle: invalidate-on-edit (reverses D-06/GEN-03), concurrency guard, batch cancel, on-demand preview endpoint (GEN-02/03/05, CFG-03) [major: tests 4, 6]
