"""Tests for the real OpenRouter analyze() wiring (Task 1) and the
multi-chunk fallback + cross-chunk reconciliation path (Task 2).

Task 1 tests are selected by `pytest -k "prompt or role or parse"` per
02-03-PLAN.md's Task 1 <verify> — they fake the OpenRouter HTTP call
entirely (via `httpx.MockTransport`) so no real network call happens even
under a non-mock LLM_BACKEND.
...
def _install_fake_openrouter(monkeypatch, fake_result: CastAnalysisResult) -> dict:
    """Intercept the outbound OpenRouter chat-completions call via
    `httpx.MockTransport` — no real network call, no dependency on any
    provider SDK, and it exercises the exact `httpx.AsyncClient` code path
    `_real_analyze` uses (unlike patching `analyze`/`_real_analyze` itself,
    which would skip over the request-construction logic under test)."""
    calls: dict = {}
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        body = {"choices": [{"message": {"content": fake_result.model_dump_json()}}]}
    ...
    monkeypatch.setattr(analysis_client.httpx, "AsyncClient", _MockedAsyncClient)
    monkeypatch.setattr(
        analysis_client,
        "settings",
        replace(settings, LLM_BACKEND="openrouter", OPENROUTER_API_KEY="fake-key"),
    )
    return calls
...
def test_real_backend_keeps_system_prompt_and_book_text_in_separate_roles(monkeypatch):
    ...
    assert calls["json"]["response_format"]["json_schema"]["schema"] == (
        CastAnalysisResult.model_json_schema()
    )
    ...
    assert messages[0]["content"] == analysis_client.CAST_ANALYSIS_SYSTEM_PROMPT
    assert book_text not in messages[0]["content"]

    # Book text arrives in a separate, later user message.
    assert messages[1]["role"] == "user"
    assert book_text in messages[1]["content"]
...
def test_real_backend_passes_continuity_context_in_user_message_not_system(monkeypatch):
    ...
    messages = calls["json"]["messages"]
    assert "Captain Reyes" not in messages[0]["content"]

    assert "Captain Reyes" in messages[1]["content"]
    assert "The sea remembers." in messages[1]["content"]
    assert "new chunk text" in messages[1]["content"]
